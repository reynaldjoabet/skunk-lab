...
/**
  * Lightweight wrapper that adds a span + latency histogram + error counter to any F[A] effect.
  */
object Instrumented {
...
// Every repo method gets a Tracer.span("RepoName.method") with relevant attributes (wallet.id, user.id, txn.reference, etc.)
// Each repo creates a repo.<name>.latency histogram and repo.<name>.errors counter at construction time
