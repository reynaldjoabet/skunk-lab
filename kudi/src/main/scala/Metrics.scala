...
object Metrics {
  ...
  private lazy val labels: MetricLabels = MetricLabels(
    forRequest = List(
      "http.request.method" -> ((req: ServerRequest) => req.method.method),
      "url.scheme"          -> ((req: ServerRequest) => req.uri.scheme.getOrElse(""))
    ),
    forEndpoint = List(
      "http.route" -> ((ep: AnyEndpoint) => ep.showPathTemplate(showQueryParam = None))
    ),
    forResponse = List(
  ...
  /**
    ...
    * or `body` - request duration is measured separately up to the point where the headers are
    * determined, and then once again when the whole response body is complete.
    */
  ...
  private def asOpenTelemetryAttributes(
    ...
    phase match {
      case Some(value) =>
        attributes :+ Attribute(AttributeKey.string(labels.forResponsePhase.name), value)
